...
def individual_deposit():
    ...
    if request.method == "POST" and form.validate():
        offer = Deposit_offer(
            ...
            surname=form.user_lastname.data,
...
def deposit_page(name):
    if name == "universal-emanet":
        deposit_idd = (
            Deposit.query.filter(Deposit.detail_slug == name).first_or_404().id
        )
        ...
        conditions = D_conditions.query.filter(
            D_conditions.deposit_id == deposit_idd
        ).all()
        ...
        if request.method == "POST":
            if request.form["select_menu_button"] == "end":
                interest_time = Interest_end.query.filter(
                    Interest_end.deposit_id == deposit_idd
                ).all()
            elif request.form["select_menu_button"] == "monthly":
                interest_time = Interest_monthly.query.filter(
                    Interest_monthly.deposit_id == deposit_idd
                ).all()
            elif request.form["select_menu_button"] == "quarterly":
                interest_time = Interest_quarterly.query.filter(
                    Interest_quarterly.deposit_id == deposit_idd
                ).all()
        else:
            interest_time = Interest_end.query.filter(
                Interest_end.deposit_id == deposit_idd
        ...
        main_office = Main_office.query.filter(
            Main_office.deposit_id == deposit_idd
        ).all()
        main_branch = Main_branch.query.filter(
            Main_branch.deposit_id == deposit_idd
        ).all()
        nesimi_branch = Nesimi_branch.query.filter(
            Nesimi_branch.deposit_id == deposit_idd
        ).all()
        nerimanov_branch = Nerimanov_branch.query.filter(
            Nerimanov_branch.deposit_id == deposit_idd
        ).all()
        ...
        return render_template(
            ...
            nesimi_branch=nesimi_branch,
            nerimanov_branch=nerimanov_branch,
        )
...
def card_offer():
    ...
    form.card_name.choices = [
        (name_.name, name_.name) for name_ in Card_name.query.all()
    ]
    form.card_type.choices = [
        (type_.type_name, type_.type_name) for type_ in Card_type.query.all()
    ]
    form.card_currency.choices = [
        (currency_.currency_name, currency_.currency_name)
        for currency_ in Card_currency.query.all()
    ]
    form.card_use_time.choices = [
        (time_.currency_time, time_.currency_time)
        for time_ in Card_use_time.query.all()
    ]
    form.card_office.choices = [
        (office_.card_office, office_.card_office)
        for office_ in Card_office.query.all()
    ]
    ...
    if request.method == "POST" and form.validate():
        card = Card_offer(
            ...
            card_currency=form.card_currency.data,
            card_use_time=form.card_use_time.data,
            card_office=form.card_office.data,
...
def homepage():
    ...
    response = requests.get(
        f"https://www.cbar.az/currencies/{datetime.now().strftime('%d.%m.%Y')}.xml"
    )
    ...
    if request.method == "POST" and form.validate():
        if current_user.is_authenticated:
            offer = Credit_offer(
                ...
                time=form.time.data,
                money=form.credit_money.data,
                interest=form.credit_interest.data,
            )
            offer.save()
            flash("Sifarişiniz qeydə alındı", "success")
            return redirect(url_for("homepage"))
    ...
    if request.method == "POST":
        ...
        if form.validate_on_submit():
            record = userque(
                ...
                xidmet_novu=form.xidmet_novu.data,
                tarix=form.tarix.data,
                mobil_nomre=form.mobil_nomre.data,
            )
            record.save()
